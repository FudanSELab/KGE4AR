...
def gene_temp_data(index, start, end):
    ...
    mcsrc.save(PathUtil.temp_multi_combine_sim_collection("{}_sample_icpc_rerank_method{}_model_{}".format(type, filter, str(index))))

def merge(thread_num):
    rerank_result = None
    for index in range(thread_num):
        if rerank_result is None:
            rerank_result: MultiCombineSimResultCollection = MultiCombineSimResultCollection.load(PathUtil.temp_multi_combine_sim_collection("{}_sample_icpc_rerank_method{}_model_{}".format(type, filter, str(index + 1))))

        else:
            rerank: MultiCombineSimResultCollection = MultiCombineSimResultCollection.load(PathUtil.temp_multi_combine_sim_collection("{}_sample_icpc_rerank_method{}_model_{}".format(type, filter, str(index + 1))))
            id2csrc = rerank.id2name2sim_collection
            rerank_result.id2name2sim_collection.update(id2csrc)

    rerank_result.save(PathUtil.multi_combine_sim_collection("{}_sample_icpc_rerank_method{}_model".format(type, filter)))
    print("save {}_sample_icpc_rerank_method{}_model success".format(type, filter))
...
if __name__ == '__main__':
    ...
    rerank_combine_sim_collection: MultiCombineSimResultCollection = MultiCombineSimResultCollection.load(PathUtil.multi_combine_sim_collection("{}_sample_icpc_rerank_method{}_model".format(type, filter)))
    ...
    for query_pair in dataset.query_data:
        ...
        for t in temp_query_pair:
            ...
            try:
                combine_results = rerank_combine_sim_collection.id2name2sim_collection[str(t["query_id"]) + "_" + str(t["target_id"])].get_combine_sim_result()
    ...
    FileUtil.write2json(os.path.join(OUTPUT_DIR, "export_result", "{}_sample_icpc_method{}_export_result.json".format(type, filter)), result_list)
    print("write ", "{}_sample_icpc_method{}_export_result.json".format(type, filter), "success")
    time3 = time.time()
    print(time1)
    print(time2)
    print(time3)
