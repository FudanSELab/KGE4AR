...
def gene_temp_data(index, start, end):
    ...
    mcsrc.save(PathUtil.temp_multi_combine_sim_collection("{}_sample_es_retrieve_method{}_model_{}".format(type, filter, str(index))))

def merge(thread_num):
    rerank_result = None
    for index in range(thread_num):
        if rerank_result is None:
            rerank_result: MultiCombineSimResultCollection = MultiCombineSimResultCollection.load(PathUtil.temp_multi_combine_sim_collection("{}_sample_es_retrieve_method{}_model_{}".format(type, filter, str(index + 1))))

        else:
            rerank: MultiCombineSimResultCollection = MultiCombineSimResultCollection.load(PathUtil.temp_multi_combine_sim_collection("{}_sample_es_retrieve_method{}_model_{}".format(type, filter, str(index + 1))))
            id2csrc = rerank.id2name2sim_collection
            rerank_result.id2name2sim_collection.update(id2csrc)

    rerank_result.save(PathUtil.multi_combine_sim_collection("{}_sample_es_retrieve_method{}_model".format(type, filter)))
    print("save {}_sample_es_retrieve_method{}_model success".format(type, filter))
...
if __name__ == '__main__':
    ...
    library2methods = FileUtil.load_data_list(
        os.path.join(DATA_DIR, "icpc_saner", "all_library_method_dic_{}.json".format(type)))

    rerank_combine_sim_collection: MultiCombineSimResultCollection = MultiCombineSimResultCollection.load(PathUtil.multi_combine_sim_collection("{}_sample_es_retrieve_method{}_model".format(type, filter)))
    ...
    FileUtil.write2json(
        os.path.join(OUTPUT_DIR, "export_result", "{}_sample_es_method{}_export_result.json".format(type, filter)),
        result_list)
    print("write ", "{}_sample_es_method{}_export_result.json".format(type, filter), "success")
    time3 = time.time()
