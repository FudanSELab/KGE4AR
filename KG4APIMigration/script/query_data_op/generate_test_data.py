...
class QueryDataGenerator():
    ...
    def clean_and_merge_data(self, old_files, new_file, new_file_name, data_type="method", split=6):
        position_a = 3
        position_b = 7
        if data_type == "method":
            position_a = 3
...
if __name__ == '__main__':
    ...
    # fileA = os.path.join(DATA_DIR, 'query_data', 'sample_apiA.txt')
    # fileB = os.path.join(DATA_DIR, 'query_data', 'sample_apiB.txt')
    # newfile = os.path.join(DATA_DIR, 'query_data', 'sample_api_{}.csv'.format(type))
    # qdg.generate_id_from_neo4j(fileA, fileB, newfile)
    # print("generate file: ", "sample_api_{}.csv".format(type))


    old_files = [os.path.join(DATA_DIR, 'query_data', 'sample_api_{}.csv'.format(type))]
    new_file = os.path.join(os.path.join(DATA_DIR, 'query_data', 'sample_api_method_{}.csv'.format(type)))
    new_file_name = os.path.join(os.path.join(DATA_DIR, 'query_data', 'sample_api_method_name_{}.csv'.format(type)))
    qdg.clean_and_merge_data(old_files, new_file, new_file_name, "method")
    print("merge file: ", "sample_api_method_{}.csv".format(type))
    print("merge file: ", "sample_api_method_name_{}.csv".format(type))
