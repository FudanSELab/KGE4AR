...
def gene_temp_data(index, start, end):
    ...
    mcsrc.save(PathUtil.temp_multi_combine_sim_collection("{}_saner_rerank_method{}_model_{}".format(type, filter, str(index))))

def merge(thread_num):
    rerank_result = None
    for index in range(thread_num):
        if rerank_result is None:
            rerank_result: MultiCombineSimResultCollection = MultiCombineSimResultCollection.load(PathUtil.temp_multi_combine_sim_collection("{}_saner_rerank_method{}_model_{}".format(type, filter, str(index + 1))))

        else:
            rerank: MultiCombineSimResultCollection = MultiCombineSimResultCollection.load(PathUtil.temp_multi_combine_sim_collection("{}_saner_rerank_method{}_model_{}".format(type, filter, str(index + 1))))
            id2csrc = rerank.id2name2sim_collection
            rerank_result.id2name2sim_collection.update(id2csrc)

    rerank_result.save(PathUtil.multi_combine_sim_collection("{}_saner_rerank_method{}_model".format(type, filter)))
    print("save {}_saner_rerank_method{}_model success".format(type, filter))
...
if __name__ == '__main__':
    ...
    #
    # mcsrc: MultiCombineSimResultCollection = MultiCombineSimResultCollection.load(PathUtil.multi_combine_sim_collection("{}_saner_method{}_model".format(type, filter)))
    # se.evaluate(dataset=dataset, top_n_list=top_n_list, multi_combine_sim_result_collection=mcsrc, combine_sim_result_collection_type="rerank", msimcol_name="{}_saner_rerank_method{}_model".format(type, filter))
    # re = [se.eval_result]
    # FileUtil.write2json(os.path.join(OUTPUT_DIR, "evaluate", "{}_saner_method{}_evaluate.json".format(type, filter)), re)
    # print("write {}_saner_method{}_evaluate success".format(type, filter))
    time3 = time.time()
    ...
    rerank_combine_sim_collection: MultiCombineSimResultCollection = MultiCombineSimResultCollection.load(
        PathUtil.multi_combine_sim_collection("{}_saner_rerank_method{}_model".format(type, filter)))
    ...
    FileUtil.write2json(
        os.path.join(OUTPUT_DIR, "export_result", "{}_saner_rerank_method{}_export_result.json".format(type, filter)),
        result_list)
    print("write ", "{}_saner_rerank_method{}_export_result.json".format(type, filter), "success")
    print(time1)
    print(time2)
    print(time3)
